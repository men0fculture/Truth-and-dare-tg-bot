...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in seen_users:
        ...
        tutorial = (
            ...
            "• Spin 🎰 to decide who starts\n"
            "• Truth 😇 → Answer or Skip\n"
            "• Dare 😈 → Do or Skip\n"
            "• Manual ✍️ → your own tasks\n\n"
            "💬 Click Answer → then type\n\n"
            "Have fun 😏🔥\n\n"
            "✨ Created by Mohit"
        )

        await update.message.reply_text(tutorial, reply_markup=get_main_keyboard())
    else:
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # RANDOM
        ...
        await send_task(context, sender, receiver, q, "😇 Random Truth", "truth")

    elif query.data == "dare":
        q = random.choice(dares)
        await send_task(context, sender, receiver, q, "😈 Random Dare", "dare")
    ...
    # DONE / SKIP
        ...
        if not sender:
            await query.edit_message_text("Expired 😶", reply_markup=get_main_keyboard())
            return

        if action == "done":
            await query.edit_message_text("Done 😌", reply_markup=get_main_keyboard())
            await context.bot.send_message(sender, "Task completed 😏🔥")

        elif action == "skip":
            await query.edit_message_text("Skipped 😅", reply_markup=get_main_keyboard())
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_state.get(user_id) == "waiting_truth":
        ...
        await context.bot.send_message(user_id, "Sent 😏", reply_markup=get_main_keyboard())
